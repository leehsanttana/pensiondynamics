...
export function MagicCard(props: MagicCardProps) {
  ...
  return (
    ...
    >
      {/* 1. Custom Holographic Border Overlay (lays precisely over the border stroke) */}
      {borderLinearGradient && (
        <motion.div
           className="absolute -inset-[1px] z-0 rounded-[inherit] transition-opacity duration-300 pointer-events-none group-hover:opacity-100 opacity-0"
           style={{
             background: borderLinearGradient,
             WebkitMask: useMotionTemplate`
               radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px, black, transparent 100%)
             `,
             mask: useMotionTemplate`
               radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px, black, transparent 100%)
             `
           }}
        />
      ...
      {/* 2. Solid interior to cover the gradient except at the 1px border edge */}
      {borderLinearGradient && (
          <div className="bg-white absolute inset-0 z-10 rounded-[inherit] pointer-events-none" />
      )}

      {/* 3. Gradient Mode Inner Spotlight */}
      {mode === "gradient" && (
        <motion.div
          suppressHydrationWarning
          className="pointer-events-none absolute inset-0 z-30 rounded-[inherit] opacity-0 transition-opacity duration-300 group-hover:opacity-100 overflow-hidden"
          style={{
            background: useMotionTemplate`
              radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
                ${gradientColor},
